...
cdef class FaustFact:
    ...
    def fact_palm4msa_gen(M, p, init_D=None):
        ...
        for i in range(0,p.num_facts):
            check_matrix(True, p.init_facts[i], message="while checking"
                         " palm4msa init facts: ")
...
def polyCoeffs(d, basisX, coeffs, dev, out=None):
    ...
    # basisX must be a numpy array in fortran column major order and dtype
    # float
    if basisX.ndim > 1:
        n = basisX.shape[1]
    else:
