...
for i in range(0,A.get_num_factors()):
    ...
    assert((A.factors(i).toarray() == list_factor_sparse[i]).all())
    #print(A.factors(i))

# test Faust saving
A.save("A.mat")
As = pyfaust.Faust(filepath="A.mat")
assert((A.factors(0).toarray() == As.factors(0).toarray()).all())
assert((A.factors(1).toarray() == As.factors(1).toarray()).all())

# test Faust transpose
#print(A.factors(0))
tA = A.transpose()
tf1 = tA.factors(1).toarray()
#print(tf1)
f1 = np.transpose(tf1)
assert(not (tf1 == A.factors(0).toarray()).all() or (tf1 == f1).all())
assert((f1 == A.factors(0).toarray()).all())
