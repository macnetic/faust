...
classdef ParamsPalm4MSA < matfaust.factparams.ParamsFact
	...
	methods
		...
		function mex_params = to_mex_struct(this, M)
			...
			% the order of struct fields doesn't matter here
			mex_params = struct('data', M, ...
				'nfacts', this.num_facts, ...
				'cons', {mex_constraints}, ...
				'init_facts', {init_facts}, ...
				'niter', this.stop_crit.num_its, ...
				'sc_is_criterion_error', this.stop_crit.is_criterion_error, ...
				'sc_error_treshold', this.stop_crit.tol, ...
				'sc_max_num_its', this.stop_crit.maxiter,'update_way', this.is_update_way_R2L, ...
				'sc_erreps', this.stop_crit.erreps, ...
