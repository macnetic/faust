...
template<typename T>
const char * palm4MSA_cu<T>::class_name="palm4MSA_cu";

template<typename T>
const T palm4MSA_cu<T>::lipschitz_multiplicator=1.001;
...
template<typename T>
palm4MSA_cu<T>::palm4MSA_cu(const faust_params<T> & params_, const cublasHandle_t cublasHandle, const bool isGlobal_) :
   data(params_.data),
   lambda(params_.init_lambda),
   nb_fact(0),
   S(params_.init_fact),
   RorL(vector<faust_cu_mat<T> >(2)),
   ind_fact(0),
   ind_ite(-1),
...
template<typename T>
palm4MSA_cu<T>::palm4MSA_cu(const faust_params_palm<T>& params_palm_,
const cublasHandle_t cublasHandle, const bool isGlobal_/*=false*/) :
   ...
   RorL(vector<faust_cu_mat<T> >(2)),
   LorR(faust_cu_mat<T>(params_palm_.init_fact[0].getNbRow())),
...
template<typename T>
void palm4MSA_cu<T>::get_facts(faust_core_cu<T> & faust_fact) const
{
	faust_core_cu<T> f(S);
	faust_fact = f;
...
template<typename T>
void palm4MSA_cu<T>::compute_projection()
{
#ifdef __COMPILE_TIMERS__
...
t_local_compute_projection.start();
   ...
   {
      //faust_cu_mat<T> matrix2project(S[ind_fact]);
      ...
      switch (const_vec[ind_fact]->getConstraintType())
      {
		...
		prox_sp_pos(S[ind_fact], constr_cast->getParameter());
         ...
         {
			throw std::logic_error("Error in palm4MSA_cu<T>::compute_projection : projection not implemented");
         ...
         case CONSTRAINT_NAME_SUPP:
         {
			...
			A.Display();
            prox_supp(S[ind_fact],(faust_cu_mat<T>)  constr_cast->getParameter());
			/*cout<<"S[ind_fact]"<<endl;
			S[ind_fact].Display();*/
         }
         break;
...
//       R of size S2xR2
   ...
   error = data;
   faust_cu_mat<T> tmp1, tmp2, tmp3, tmp4;
...
sprintf(nomFichier,"error_1_%d_device.tmp",cmpt);*/
      ...
      {
         // tmp1 = L*S
         multiply(RorL[ind_fact], S[ind_fact], tmp1, cublas_handle);
         // error = lambda*tmp1*R - error (= lambda*L*S*R - data )
         gemm<T>(tmp1, LorR, error, lambda, -1.0, 'N', 'N', cublas_handle);
      }
   }
...
sprintf(nomFichier,"error_1_%d_device.tmp",cmpt);*/
      ...
      {
         // tmp1 = S*R
         multiply(S[ind_fact], LorR, tmp1, cublas_handle);
         // error = lambda*L*tmp1 - error (= lambda*L*S*R - data )
         gemm<T>(RorL[ind_fact], tmp1, error, lambda, -1.0, 'N', 'N', cublas_handle);
      }
   }
   ...
   if (idx==0 || idx==2) // computing L'*error first, then (L'*error)*R'
   {
      ...
      {
         // tmp3 = lambda*L'*error (= lambda*L' * (lambda*L*S*R - data) )
         gemm<T>(RorL[ind_fact], error, tmp3, lambda, 0.0, 'T', 'N', cublas_handle);
         // grad_over_c = 1/c*tmp3*R' (= 1/c*lambda*L' * (lambda*L*S*R - data) * R' )
         gemm<T>(tmp3, LorR, grad_over_c, 1.0/c, 0.0,'N','T', cublas_handle);
      }
   }
   else // computing error*R' first, then L'*(error*R')
   {
      ...
      {
         // tmp3 = lambda*error*R' (= lambda * (lambda*L*S*R - data) * R' )
         gemm<T>(error, LorR, tmp3, lambda, 0.0, 'N', 'T', cublas_handle);
         // grad_over_c = 1/c*L'*tmp3 (= 1/c*L' * lambda*(lambda*L*S*R - data) * R' )
         gemm<T>(RorL[ind_fact], tmp3, grad_over_c, 1.0/c, 0.0,'T','N', cublas_handle);
      }
...
template<typename T>
void palm4MSA_cu<T>::compute_lambda()
{
#ifdef __COMPILE_TIMERS__
...
t_local_compute_lambda.start();
   ...
   // Xt_Xhat = data'*X_hat
   faust_cu_mat<T> Xt_Xhat;
   gemm<T>(data, LorR, Xt_Xhat, 1.0, 0.0, 'T','N', cublas_handle);

   // Xhatt_Xhat = X_hat'*X_hat
   faust_cu_mat<T> Xhatt_Xhat;
   gemm<T>(LorR, LorR, Xhatt_Xhat, 1.0, 0.0, 'T','N', cublas_handle);
...
t_local_update_R.start();
   ...
   if (!isUpdateWayR2L)
   {
      ...
      for (int i=nb_fact-2 ; i>-1 ; i--)
         //  R[i] = S[i+1] * R[i+1]
         gemm(S[i+1], RorL[i+1], RorL[i], cublas_handle);
   }
   else
   {
...
t_local_update_L.start();
   ...
   else
   {
      ...
      for (int i=0 ; i<nb_fact-1 ; i++)
         ...
         gemm(RorL[i] , S[i], RorL[i+1], cublas_handle);
   }
#ifdef __COMPILE_TIMERS__
...
template<typename T>
void palm4MSA_cu<T>::check_constraint_validity()
{
#ifdef __COMPILE_TIMERS__
...
template<typename T>
void palm4MSA_cu<T>::init_fact(int nb_facts_)
{
#ifdef __COMPILE_TIMERS__
...
t_local_init_fact.start();
  ...
  /*if(isInit && isGlobal)
  {
     cerr << "Error in palm4MSA_cu<T>::init_fact : global factorization has already been initialized" <<endl;
     exit(EXIT_FAILURE);
  }
  else if (isGlobal)
  {
...
template<typename T>
void palm4MSA_cu<T>::init_fact_from_palm(const palm4MSA_cu& palm2, bool isFactSideLeft)
{
#ifdef __COMPILE_TIMERS__
...
template<typename T> int palm4MSA_cu<T>::nb_call_prox_spcol;
template<typename T> int palm4MSA_cu<T>::nb_call_prox_splin;
template<typename T> int palm4MSA_cu<T>::nb_call_prox_normcol;




template<typename T>
void palm4MSA_cu<T>::init_local_timers()
{
...
template<typename T>
void palm4MSA_cu<T>::print_global_timers()const
{
...
template<typename T>
void palm4MSA_cu<T>::print_prox_timers() const
{
/*	cout << "prox timers in palm4MSA_cu : " << endl;
...
template<typename T>
void palm4MSA_cu<T>::print_local_timers()const
{
   cout << "timers in palm4MSA_cu : " << endl;
   ...
   cout << "t_check                     = " << t_local_check.get_time()               << " s for "<< t_local_check.get_nb_call()               << " calls" << endl;
