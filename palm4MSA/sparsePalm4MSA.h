#ifndef __FAUST_SPARSEPALM4MSA_H__
#define __FAUST_SPARSEPALM4MSA_H__
...
class sparsePalm4MSA
{
   public:
      //palm4MSA(const faust_params& params_, const bool isGlobal_);
      sparsePalm4MSA(const faust_params_palm& params_palm_);
	  sparsePalm4MSA(const faust_params& params_, const bool isGlobal_);
	  void next_step();	
      void set_constraint(const std::vector<const faust_constraint_generic*> const_vec_){const_vec=const_vec_;isConstraintSet=true;nb_fact=const_vec.size();}
      ...
      void update_lambda_from_palm(const sparsePalm4MSA& palm){lambda *= palm.lambda;}
	  //void compute_last_update();
      void compute_facts();
	  faust_spmat get_fact(const int id_fact) const;
	  faust_spmat get_residuum(const bool isFactSideLeft) const;
      ...
      void init_fact_from_palm(const sparsePalm4MSA& palm2, bool isFactSideLeft);
