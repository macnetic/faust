...
template<typename T>
faust_cu_vec<T> operator*(const faust_core<T>& f, const faust_cu_vec<T>& v);
template<typename T>
faust_cu_mat<T> operator*(const faust_core<T>& f, const faust_cu_mat<T>& M);
...
class faust_core
{
	public:
		faust_core();
		faust_core(const std::vector<faust_cu_spmat<T> >& facts, const T lambda_ = (T)1.0);
		faust_core(const faust_core<T> & A);
		faust_core(const std::vector<faust_cu_mat<T> >&facts);
	...
	friend faust_cu_vec<T> operator*<>(const faust_core<T>& f, const faust_cu_vec<T>& v);
	friend faust_cu_mat<T> operator*<>(const faust_core<T>& f, const faust_cu_mat<T>& M);
	// friend void multiply<>(const faust_core<T> & A, const faust_cu_mat<T> & B, faust_cu_mat<T> & C,const T & alpha, char typeA, char typeMult);
