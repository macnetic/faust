...
template<typename T>
void write_faust_spmat_into_matfile(const faust_spmat<T>& M, const char* fileName, const char* variableName)
{
...
{
	vector< faust_mat<T> > list_dense_mat;
...
template<typename T>
void init_faust_mat_vector_from_matiofile( vector<faust_mat<T> > & vec_M, const char* fileName, const char* variableName)
{
	...
	faust_mat<T> current_fact;
	vec_M.resize(0);


	if (facts_var->class_type != MAT_C_CELL)
	{
		cerr << "error in init_faust_mat<T>_vector_from_matiofile : facts is'nt a cell_array" << endl;
		exit(EXIT_FAILURE);
	}
	...
	for (int j=0;j<nbr_facts;j++)
	{
		...
		for (size_t i = 0 ; i < current_fact_var->dims[0] * current_fact_var->dims[1] ; i++)
      	{
			(((current_fact.getData()))[i]) = (T) (((double*)(current_fact_var->data))[i]);
...
void init_mat_from_matvar(faust_mat<T> & M,matvar_t* var)
{
	...
	{
		(((M.getData()))[k]) = (T) (((double*)(var->data))[k]);

	}
}

template<typename T>
void init_spmat_from_matvar(faust_spmat<T>& S, matvar_t* var)
{
   mat_sparse_t* mat_sparse = (mat_sparse_t*)var->data;
   if( var->class_type != MAT_C_SPARSE
       || var->rank != 2)
   {
      cerr << "error in init_faust_mat<T>_from_matio : "<< "the variable seems not to be a double sparse matrix." << endl;
      exit(EXIT_FAILURE);
   }
   ...
   {
      cerr<<"Error in init_faust_spmat<T>_from_matio : incorrect dimensions"<<endl;
      exit(EXIT_FAILURE);
   }

   vector<int> rowind(mat_sparse->ndata);
   vector<int> colind(mat_sparse->ndata);
   vector<T> values(mat_sparse->ndata);


   for (size_t i = 0 ; i < mat_sparse->ndata ; i++)
      values[i] = (T) (((double*)(mat_sparse->data))[i]);
   ...
   S = faust_spmat<T>(rowind, colind, values, var->dims[0], var->dims[1]);

   if (cmpt != S.getNonZeros())
   {
      cerr<<"Error in init_faust_spmat<T>_from_matio : cmpt != nnz : cmpt="<<cmpt<<" ; nnz="<<S.getNonZeros()<<endl;
      exit(EXIT_FAILURE);
   }
