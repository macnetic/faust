...
namespace Faust
{
		...
		TransformHelper<FPP,GPU2>::TransformHelper(const std::vector<MatGeneric<FPP,GPU2> *>& facts, const FPP lambda_/*= (FPP)1.0*/, const bool optimizedCopy/*=false*/, const bool cloning_fact /*= true*/, const bool internal_call/*=false*/) : TransformHelper<FPP,GPU2>()
	{
		...
		this->transform->scalarMultiply(lambda_, min_size_id);
	}
	...
	template<typename FPP>
		...
		TransformHelper<FPP2,GPU2>* TransformHelper<FPP,GPU2>::cast()
		{
			this->eval_sliced_Transform();
			this->eval_fancy_idx_Transform();
			std::vector<MatGeneric<FPP2,GPU2>*> cast_data;
			...
			for(auto curfac: this->transform->data)
			{
				...
				{
					auto cast_fac = new MatDense<FPP2,GPU2>(curfac->getNbRow(), curfac->getNbCol(), /* cpu_data*/ nullptr, /* no_alloc*/ false, /* dev_id*/ -1, /* stream*/ nullptr);
					*cast_fac = curfac_ds->template cast<FPP2>();
					cast_data.push_back(cast_fac);
				}
				else if(curfac_sp = dynamic_cast<MatSparse<FPP, GPU2>*>(curfac))
				{
					auto cast_fac = new MatSparse<FPP2,GPU2>(curfac->getNbRow(), curfac->getNbCol(), /* nnz*/ 0, /* values */ nullptr, /* rowptr*/ nullptr, /* colinds*/ nullptr);
					*cast_fac = curfac_sp->template cast<FPP2>();
					cast_data.push_back(cast_fac);
				}
				else
				{
			...
			return new TransformHelper<FPP2, GPU2>(cast_data, 1.0, false, false, true);
		}
	 ...
	 *  \param factors must be a vector<MatGeneric<FPP,GPU2>*> of all factors to transform to MatButterfly/MatPerm,
...
#define optButterfly_factors(factors, has_perm, src_factors) \
	...
	for(int i = 0; i < n__; i++)\
	{\
		factors[i] = new MatButterfly<FPP, GPU2>(*dynamic_cast<MatSparse<FPP, GPU2>*>(src_factors[i]), i);\
	}\
	if(has_perm) factors[n__] = new MatPerm<FPP, GPU2>(*dynamic_cast<MatSparse<FPP, GPU2>*>(src_factors[n__]));\
	...
	template<typename FPP>
		...
		{ //TODO: factorize with CPU code
		  ...
		  // TODO: what if F's state is special (transpose, conjugate, sliced, indexed)?
			bool has_perm = false;
			const MatSparse<FPP, GPU2>* last_sp_mat = nullptr;
			// test if last_perm is a valid permutation (if not it is assumed that it's a butterfly matrix)
			if(last_sp_mat = dynamic_cast<MatSparse<FPP, GPU2>*>(F->transform->data[F->size()-1]))
				has_perm = MatPerm<FPP, GPU2>::isPerm(*last_sp_mat, /*verify_ones*/ false);
			std::vector<MatGeneric<FPP,GPU2>*> factors(F->size());
			optButterfly_factors(factors, has_perm, F->transform->data);
			auto oF = new TransformHelper<FPP, GPU2>(factors, FPP(1.0), false, false, /* internal call */ true);
