...
//		slack.setConstant(1e-10);
		...
		while(true)
		{
				...
				for(int side=0;side<2;side++)
				{
					for(int vertex=0; vertex < size; vertex++)
						if(! visited(side, vertex))
							delta = min(Faust::fabs(delta), Faust::fabs(slack(side, vertex)));
				}
				set_all_to_self_minus(slack, FPP(delta));
				for(int vertex=0; vertex < size; vertex++)
				{
					if(visited(0,vertex))
			...
			{
				FPP p_diff = FPP(weight(u)) - potential(side, vertex) - potential(1 - side, u);
