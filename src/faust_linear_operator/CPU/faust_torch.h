...
{
	/**
	 * Converts a MatDense to a torch::Tensor.
	 *
	...
	template<typename FPP, FDevice D>
		void convMatDenseToTensor(const MatDense<FPP,D> & dm, torch::Tensor & t, at::DeviceType dev = at::kCPU, const bool clone = false, const bool transpose = true);

	/**
	 * Converts a torch::Tensor (t) to MatDense (dm).
	 *
	 * \param transpose see convMatDenseToTensor.
	 *
	 */
	template<typename FPP, FDevice D>
		void convTensorToMatDense(const torch::Tensor & t, MatDense<FPP,D> & dm, const bool transpose = true);


	/**
	 * Converts a MatSparse to a torch::Tensor.
	 *
	 * \note Complex tensors are not available in libtorch, an exception is thrown when FPP is complex.
	 *
	 *\param dev the device at::kCPU or at::kCUDA.
	 ...
	 */
	template<typename FPP, FDevice D>
		void convMatSparseToTensor(const MatSparse<FPP,D> & spm, torch::Tensor & t, at::DeviceType dev, const bool clone, const bool transpose = true);
	/**
	 * Converts a MatGeneric vector to a torch::TensorList (vector alias).
	 *
	 *\param dev the device at::kCPU or at::kCUDA.
	 ...
	 */
	template<typename FPP, FDevice D>
		void convMatGenListToTensorList(const std::vector<MatGeneric<FPP,D>*> & ml, std::vector<torch::Tensor> &tl, at::DeviceType dev = at::kCPU, const bool clone = false, const bool transpose = true);
	 ...
	 * Returns the result as a MatDense.
	 */
	template<typename FPP, FDevice D>
		void tensor_chain_mul(const std::vector<MatGeneric<FPP,D>*>& ml, MatDense<FPP,D> & out, const MatGeneric<FPP,D>* op = nullptr, const bool on_gpu = false, const bool clone = false, const bool chain_opt = false,  const bool contiguous_dense_to_torch = false, const bool transpose = true);
	 ...
	 */
	template<typename FPP, FDevice D>
		void tensor_chain_mul(const std::vector<torch::Tensor>& tl, MatDense<FPP,D> & out, const MatGeneric<FPP,D>* op, const bool on_gpu, const bool clone, const bool chain_opt, const bool contiguous_dense_to_torch, const bool transpose = true);
