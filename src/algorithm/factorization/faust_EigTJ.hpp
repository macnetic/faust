...
#define calc_theta_err(theta) (*this->L)(this->p,this->q)*cos(2*theta) + 0.5*((*this->L)(this->q,this->q) - (*this->L)(this->p,this->p))*sin(2*theta)
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L(MatDense<FPP,Cpu> & L)
		{
			// L = S'*L*S
#ifdef DEBUG_GIVENS
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L(MatSparse<FPP,Cpu> & L)
		{
...
#undef OPT_UPDATE_SPARSE_L
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L_first(Vect<FPP,DEVICE>& L_vec_p, Vect<FPP,DEVICE>& L_vec_q, const FPP2& c, const FPP2& s, int p, int q, MatDense<FPP,DEVICE> & L)
		{
#define copy_vec2Lrow(vec,rowi) \
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L_second(Vect<FPP,DEVICE>& L_vec_p, Vect<FPP,DEVICE>& L_vec_q, const FPP2& c, const FPP2& s, int p, int q, MatDense<FPP,DEVICE> & L)
		{
			Vect<FPP,DEVICE> tmp, tmp2;
			/*========== L *= S */
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L_second(Eigen::SparseMatrix<FPP,Eigen::RowMajor > & L_vec_p, Eigen::SparseMatrix<FPP, Eigen::RowMajor>& L_vec_q, const FPP2& c, const FPP2& s, int p, int q, MatSparse<FPP,DEVICE> & L)
		{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJ<FPP,DEVICE,FPP2>::update_L_first(Eigen::SparseMatrix<FPP, Eigen::RowMajor> & L_vec_p, Eigen::SparseMatrix<FPP, Eigen::RowMajor>& L_vec_q, const FPP2& c, const FPP2& s, int p, int q, MatSparse<FPP,DEVICE> & L)
		{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		const MatSparse<FPP,DEVICE> EigTJ<FPP,DEVICE,FPP2>::get_Dspm(const bool ord /* default to false */)
		{
