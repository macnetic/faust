namespace Faust
{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L(MatDense<FPP,Cpu> & L)
		{
			// L = S'*L*S
#ifdef DEBUG_GIVENS
			//MatDense<FPP,Cpu> L_copy = L;
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L(MatSparse<FPP,Cpu> & L)
		{
...
#undef OPT_UPDATE_SPARSE_L
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L_first(Vect<FPP,DEVICE>& L_vec_p, Vect<FPP,DEVICE>& L_vec_q, const FPP& c_pp, const FPP& c_pq, const FPP& c_qp, const FPP& c_qq, int p, int q, MatDense<FPP,DEVICE> & L)
		{
#define copy_vec2Lrow(vec,rowi) \
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L_second(Vect<FPP,DEVICE>& L_vec_p, Vect<FPP,DEVICE>& L_vec_q, const FPP& c_pp, const FPP& c_pq, const FPP& c_qp, const FPP& c_qq, int p, int q, MatDense<FPP,DEVICE> & L)
		{
			Vect<FPP,DEVICE> tmp, tmp2;
			/*========== L *= S */
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L_second(Eigen::SparseMatrix<FPP,Eigen::RowMajor > & L_vec_p, Eigen::SparseMatrix<FPP, Eigen::RowMajor>& L_vec_q, const FPP& c_pp, const FPP& c_pq, const FPP& c_qp, const FPP& c_qq, int p, int q, MatSparse<FPP,DEVICE> & L)
		{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		void EigTJComplex<FPP,DEVICE,FPP2>::update_L_first(Eigen::SparseMatrix<FPP, Eigen::RowMajor> & L_vec_p, Eigen::SparseMatrix<FPP, Eigen::RowMajor>& L_vec_q, const FPP& c_pp, const FPP& c_pq, const FPP& c_qp, const FPP& c_qq, int p, int q, MatSparse<FPP,DEVICE> & L)
		{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		EigTJComplex<FPP,DEVICE,FPP2>::EigTJComplex(MatSparse<FPP,DEVICE>& Lap, int J, unsigned int verbosity /* deft val == 0 */, const double stoppingError /* default to 0.0 */, const bool errIsRel, const bool enable_large_Faust, const int err_period/*=100*/) : EigTJGen<typename FPP::value_type, DEVICE, FPP2, FPP>(Lap, J, verbosity, stoppingError, errIsRel, enable_large_Faust, err_period), C(Lap.getNbRow(), Lap.getNbCol())
	{
		//see parent ctor
		this->C.setZeros();
	}
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		EigTJComplex<FPP,DEVICE,FPP2>::EigTJComplex(MatDense<FPP,DEVICE>& Lap, int J, unsigned int verbosity /* deft val == 0 */, const double stoppingError, const bool errIsRel, const bool enable_large_Faust, const int err_period/*=100*/) : EigTJGen<typename FPP::value_type, DEVICE, FPP2, FPP>(Lap, J, verbosity, stoppingError, errIsRel, enable_large_Faust, err_period), C(Lap.getNbRow(), Lap.getNbCol())
	{
		// see parent ctor
		this->C.setZeros();
	}
	...
	template<typename FPP, FDevice DEVICE, typename FPP2>
		const MatSparse<FPP,DEVICE> EigTJComplex<FPP,DEVICE,FPP2>::get_Dspm(const bool ord /* default to false*/)
		{
