...
namespace Faust
{
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		EigTJParallelGen<FPP,DEVICE,FPP2,FPP4>::EigTJParallelGen(int t, EigTJGen<FPP, DEVICE, FPP2, FPP4> & alg) : alg(alg), t(t), fact_nrots(0)
	{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		void EigTJParallelGen<FPP,DEVICE,FPP2,FPP4>::update_L(MatSparse<FPP,Cpu> & L)
		{
			// L = S'*L*S
...
#pragma omp barrier //S'*L must be computed before starting L*S computation
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		void EigTJParallelGen<FPP,DEVICE,FPP2,FPP4>::finish_fact()
		{
			...
			alg.facts[alg.ite] = MatSparse<FPP4,DEVICE>(alg.fact_mod_row_ids, alg.fact_mod_col_ids, alg.fact_mod_values, n, n);
