...
const char * Palm4MSA<FPP,DEVICE>::class_name="Palm4MSA";

template<typename FPP,Device DEVICE>
const FPP Palm4MSA<FPP,DEVICE>::lipschitz_multiplicator=1.001;


template<typename FPP,Device DEVICE>
Palm4MSA<FPP,DEVICE>::Palm4MSA(const Faust::Params<FPP,DEVICE> & params_,const BlasHandle<DEVICE> blasHandle, const bool isGlobal_) :
    data(params_.data),
    ...
    blas_handle(blasHandle)
{
   ...
   if(isGlobal)
      stop_crit = StoppingCriterion<FPP>(params_.stop_crit_global);
   else
      stop_crit = StoppingCriterion<FPP>(params_.stop_crit_2facts);
...
template<typename FPP,Device DEVICE>
Palm4MSA<FPP,DEVICE>::Palm4MSA(const Faust::ParamsPalm<FPP,DEVICE>& params_palm_,const BlasHandle<DEVICE> blasHandle,const bool isGlobal_/*=false*/) :
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::compute_facts()
{
	while (do_continue())
	{
		next_step();
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::get_facts(Faust::Transform<FPP,DEVICE> & faust_fact) const
{
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::compute_projection()
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::compute_grad_over_c()
{
/*static int cmpt = -1;
cmpt++;
char nomFichier[100];*/
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::compute_lambda()
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::update_R()
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::update_L()
{
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::check_constraint_validity()
{
#ifdef __COMPILE_TIMERS__
...
#ifdef __PAS_FIXE__
// Palm4MSA<FPP,DEVICE>::compute_c() has been defined as an inline method in Palm4MSA.h
#else
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::compute_c()
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::init_fact(int nb_facts_)
{
#ifdef __COMPILE_TIMERS__
...
t_local_init_fact.start();
  ...
  /*if(isInit && isGlobal)
  {
     cerr << "Error in Palm4MSA<FPP,DEVICE>::init_fact : global factorization has already been initialized" <<endl;
     exit(EXIT_FAILURE);
  }
  else if (isGlobal)
  {
 ...
 */
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::next_step()
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::init_fact_from_palm(const Palm4MSA& palm2, bool isFactSideLeft)
{
#ifdef __COMPILE_TIMERS__
...
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_compute_projection;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_compute_grad_over_c;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_compute_c;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_compute_lambda;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_update_R;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_update_L;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_check;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_init_fact;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_next_step;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_global_init_fact_from_palm;


template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_prox_const;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_prox_sp;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_prox_spcol;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_prox_splin;
template<typename FPP,Device DEVICE> Faust::Timer Palm4MSA<FPP,DEVICE>::t_prox_normcol;

template<typename FPP,Device DEVICE> int Palm4MSA<FPP,DEVICE>::nb_call_prox_const;
template<typename FPP,Device DEVICE> int Palm4MSA<FPP,DEVICE>::nb_call_prox_sp;
template<typename FPP,Device DEVICE> int Palm4MSA<FPP,DEVICE>::nb_call_prox_spcol;
template<typename FPP,Device DEVICE> int Palm4MSA<FPP,DEVICE>::nb_call_prox_splin;
template<typename FPP,Device DEVICE> int Palm4MSA<FPP,DEVICE>::nb_call_prox_normcol;




template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::init_local_timers()
{
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::print_global_timers()const
{
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::print_prox_timers() const
{
...
template<typename FPP,Device DEVICE>
void Palm4MSA<FPP,DEVICE>::print_local_timers()const
{
    cout << "timers in Palm4MSA : " << endl;
