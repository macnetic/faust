...
namespace Faust
{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		EigTJGen<FPP,DEVICE,FPP2,FPP4>::EigTJGen(MatSparse<FPP4, DEVICE> & Lap, int J, unsigned int verbosity /* deft val == 0 */, const double stoppingError, const bool errIsRel, const bool enable_large_Faust/* deft to false */, const int err_period/*=100*/) :  EigTJGen<FPP,DEVICE,FPP2,FPP4>(&Lap, J, verbosity, stoppingError, errIsRel, enable_large_Faust, err_period)
	{
		L = new MatSparse<FPP4,DEVICE>(Lap);
	}

	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		EigTJGen<FPP,DEVICE,FPP2,FPP4>::EigTJGen(MatDense<FPP4, DEVICE> & Lap, int J, unsigned int verbosity /* deft val == 0 */, const double stoppingError, const bool errIsRel, const bool enable_large_Faust/* deft to false */, const int err_period/*=100*/) : EigTJGen<FPP,DEVICE,FPP2,FPP4>(&Lap, J, verbosity, stoppingError, errIsRel,  enable_large_Faust, err_period)
	{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const Vect<FPP,DEVICE>& EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_D(const bool ord /* default to false */)
		{
			if(ord)
			{
				if(!is_D_ordered)
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const Vect<FPP,DEVICE>& EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_D(const int ord /* default to false */)
		{
			if(ord != 0)
			{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		...
		void EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_Dspm(MatSparse<FPP3,DEVICE> & spD, const bool ord /* default to false */)
		{
			const Vect<FPP,DEVICE>& D_ = this->get_D(ord);
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		...
		{
			const Vect<FPP,DEVICE>& D_ = get_D(ord);
			const FPP* src_data_ptr = D_.getData();
			memcpy(diag_data, src_data_ptr, sizeof(FPP)*D_.size());
		}

	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const MatDense<FPP4,DEVICE> EigTJGen<FPP,DEVICE,FPP2,FPP4>::compute_fourier(const bool ord /* default to false */)
		{
			MatDense<FPP4,Cpu> fourier(L->getNbRow(), L->getNbCol());
			MatDense<FPP4,Cpu>* ord_fourier;
			fourier.setEyes();
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const MatGeneric<FPP,DEVICE>& EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_L() const
		{
			return *L;
		}
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const MatDense<FPP4,DEVICE>& EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_Lap() const
		{
			return Lap;
		}

	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		const vector<MatSparse<FPP4,DEVICE>>& EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_facts() const
		{
			return facts;
		}

	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		Transform<FPP4,DEVICE> EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_transform(const bool ord)
		{
	...
	template<typename FPP, FDevice DEVICE, typename FPP2, typename FPP4>
		Transform<FPP4,DEVICE> EigTJGen<FPP,DEVICE,FPP2,FPP4>::get_transform(const int ord, const bool copy/*=true*/, const int first_nfacts/*=-1*/)
		{
			...
			auto t = Transform<FPP4,DEVICE>(facts, /* lambda */ FPP4(1.0), /* optimizedCopy */false, /* cloning_fact */ copy);
