...
namespace Faust {
    template<typename FPP,FDevice DEVICE>
        void init_params_palm_from_matiofile(ParamsPalm<FPP,DEVICE>& params,const char* fileName, const char* variableName)
        {
            ...
            MatDense<FPP,DEVICE> data_mat,current_fact;
            vector<MatDense<FPP,DEVICE> > init_facts;
            ...
            for (int i=0;i<nbr_params;i++)
            {
                ...
                if (strcmp(current_fieldName,"init_facts")==0)
                {
                    ...
                    for (int j=0;j<(current_var->dims[1]);j++)
                    {
                        ...
                        init_facts.push_back(MatDense<FPP,DEVICE>(current_fact));
                    }
                    params.init_fact=init_facts;
                }
                if (strcmp(current_fieldName,"cons")==0)
                {
                    vector<const ConstraintGeneric*> consS;
        ...
        void init_params_from_matiofile(Params<FPP,DEVICE, FPP2>& params, const char* fileName, const char* variableName)
        {
            ...
            for (int i=0;i<nbr_params;i++)
            {
                ...
                if (strcmp(current_fieldName,"niter1")==0)
                {
                    //cout<<"niter1="<<endl;
                    niter=(int)((double*)(current_var->data))[0];
                    StoppingCriterion<FPP2> stop_cri(niter);
                ...
                {
                    vector<const ConstraintGeneric*> consS;
                    vector<vector<const ConstraintGeneric*> > consSS;
                    ...
                    for (int ll=0;ll<(current_var->dims[0]);ll++)
                    {
                        ...
                        for (int L=0;L<consS.size();L++)
                        {
                            ...
                            //ConstraintInt* const_int = (ConstraintInt*)(params.cons[L]);
        ...
        void add_constraint(std::vector<const ConstraintGeneric*> & consS,matvar_t* cons_var)
        {
            ...
            switch(const_type)
            {
                ...
                case 0:
                    ...
                    consS.push_back(new ConstraintInt<FPP,DEVICE>(cons_name,int_parameter,cons_dim1,cons_dim2));
                    break;
                ...
                case 1 :
                    ...
                    consS.push_back(new ConstraintFPP<FPP,DEVICE,FPP2>(cons_name,real_parameter,cons_dim1,cons_dim2));
                    break;

                    // CASE MAT CONSTRAINT
                case 2 :
                    MatDense<FPP,Cpu> mat_parameter;
                    cons_field_var=Mat_VarGetCell(cons_var,1);
                    ...
                    consS.push_back(new ConstraintMat<FPP,DEVICE>(cons_name,mat_parameter,cons_dim1,cons_dim2));
                    break;
            }
    ...
    template<typename FPP,FDevice DEVICE, typename FPP2>
        void Display_params(Params<FPP,DEVICE,FPP2> & params)
        {
            ...
            for (int L=0;L<params.cons[0].size();L++)
            {
                ...
                {	//string type_cons;
                    ...
                    {
                        ConstraintInt<FPP,DEVICE>* const_int = (ConstraintInt<FPP,DEVICE>*)(params.cons[jl][L]);
                        cout<<" parameter :"<<(*const_int).get_parameter()<<endl;
                    }

                    else if (params.cons[jl][L]->is_constraint_parameter_real())
                    {
                        ConstraintFPP<FPP,DEVICE,FPP2>* const_real = (ConstraintFPP<FPP,DEVICE,FPP2>*)(params.cons[jl][L]);
                        cout<<" parameter :"<<(*const_real).get_parameter()<<endl;
                    }

                    else if (params.cons[jl][L]->is_constraint_parameter_mat())
                    {
                        ConstraintMat<FPP,DEVICE>* const_mat = (ConstraintMat<FPP,DEVICE>*)(params.cons[jl][L]);
                        cout<<" parameter :"<<endl;
